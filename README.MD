...
使用 FastAPI 作為後台管理 API，LINE Bot 作為前台報名介面的 Good Job 報名系統。

## 功能特色

### 後台功能（FastAPI）
1. **使用者認證** - JWT 認證機制，支援登入和註冊
2. **權限管理** - 區分管理員和一般使用者權限
3. **發佈工作** - 透過 API 發佈新的 Good Job（需要管理員權限）
4. **查詢工作** - 取得所有工作或特定工作資訊（需要認證）
5. **查詢報名清單** - 取得特定工作的所有報名記錄（需要管理員權限）
6. **查詢所有報名** - 取得系統中所有報名記錄（需要管理員權限）
7. **使用者管理** - 管理使用者帳號（需要管理員權限）

### 前台功能（LINE Bot）
1. **LINE 使用者註冊** - 透過 LINE 進行註冊（不需要密碼）
2. **查看工作列表** - 顯示所有可報名的工作（日期大於等於今天）
3. **工作詳情** - 查看工作詳細資訊（包含地點圖片）
4. **報名功能** - 選擇班別並完成報名（僅限已註冊使用者）
5. **取消報名** - 取消已報名的工作（含確認流程）
6. **查詢已報名** - 查看自己的所有報名記錄
...
2. **ApplicationService** - 報名管理服務
   - 建立、查詢、取消報名
   - 自動產生報名編號（格式：工作編號-日期-流水號，例如：JOB001-20260110-001）
   - 支援使用者報名索引和工作報名索引
...
**設定 JWT Secret Key（用於 API 認證）：**
```bash
export JWT_SECRET_KEY="your-secret-key-here"
```

**設定 Google Maps API Key（用於取得座標）：**
```bash
...
- **資料持久化：**
  - 目前資料儲存在記憶體中，容器重啟後會遺失
  - 如需持久化，建議使用資料庫（PostgreSQL、MySQL 等）
  - 可在 `docker-compose.yml` 中取消註解 volume 掛載來持久化資料
...
- **註冊** / **register** / **Register** / **REGISTER** - 註冊為系統使用者
...
- **LINE 使用者註冊**：透過 LINE Bot 註冊，不需要密碼
- **註冊後才能報名**：只有已註冊的使用者才能報名工作
- **多步驟註冊**：需要填寫姓名、手機、地址、Email（Email 可選）
- **資料驗證**：手機號碼會驗證格式（台灣手機號碼：10位數，09開頭）
- **取消註冊**：在註冊過程中輸入「取消」可隨時取消註冊流程
- **使用者名稱**：直接使用 LINE User ID 作為使用者名稱（不另外產生）

### 操作流程

#### 註冊流程
1. 發送「註冊」或點擊主選單的「📝 註冊」按鈕
2. 依序輸入以下資料：
   - **第一步：姓名** - 輸入您的姓名
   - **第二步：手機號碼** - 輸入10位數手機號碼（例如：0912345678）
   - **第三步：地址** - 輸入您的地址
   - **第四步：Email** - 輸入 Email（可選，輸入「跳過」即可略過）
3. 系統顯示註冊成功訊息，包含所有註冊資訊

#### 報名流程
1. **首次使用請先註冊**：發送「註冊」完成註冊
2. 發送「工作列表」或點擊主選單的「查看工作列表」
3. 系統顯示所有可報名的工作（卡片式顯示，包含工作狀態）
4. 點擊「查看詳情」查看工作詳細資訊（包含地點圖片）
5. 點擊「報名」按鈕（未註冊使用者會看到註冊提示）
6. 選擇要報名的班別（最多顯示 4 個班別）
7. 系統顯示報名成功訊息，包含報名編號

**注意**：未註冊的使用者無法報名工作，系統會提示先完成註冊。
...
### Application（報名記錄）
- `id`: 報名ID（自動產生，格式：工作編號-日期-流水號，例如：JOB001-20260110-001）
- `job_id`: 工作ID
